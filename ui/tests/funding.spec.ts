...
test("funding page uses the verified sponsor destination without automatic outbound requests", async ({
  ...
  await expect(sponsor).toHaveAttribute(
    "href",
    "https://github.com/sponsors/Harshil-Anuwadia",
  );
...
test("business inquiry is previewed locally, downloadable and invalidated when edited", async ({
  page,
}) => {
  const writes = await mockApi(page);
  ...
  await page
    .getByLabel("What would you like to accomplish?")
    .fill("Package a reproducible Linux deployment with acceptance tests.");
  ...
  expect(url.origin + url.pathname).toBe(
    "https://github.com/Harshil-Anuwadia/aegisdns/issues/new",
  );
  ...
  await expect(page.getByLabel("Draft text")).toHaveValue(
  ...
  await page
    ...
    .fill("Revised organization");
  await expect(
    page.getByRole("link", { name: "Review draft on GitHub" }),
  ).toHaveCount(0);
  expect(writes).toEqual([]);
});
