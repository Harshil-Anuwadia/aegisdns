...
export default function Network() {
  ...
  const query = useApi<Graph>(
    `/graph?hours=${hours}&domain=${encodeURIComponent(domain)}`,
    15000,
  );
  const layout = useMemo(() => {
    ...
    const nodes = raw.map((n) => {
      ...
      return {
        id: n.id,
        type: "relationship",
        position: { x: column * 315, y: row * 95 },
        data: {
          label: n.label,
          kind: n.kind,
          count: edges
  ...
  return (
    <>
      ...
      <p className="footnote">
        ...
        identity. Showing up to 140 nodes and 280 links. ASN and country data
        require a local enrichment file; certificates appear only when TLSA
